...
axios.get(`${APP_URL}/api/student`)
.then((res)=> {
    data.value = res.data.student
})
.catch((erreur) => console.log(erreur))


const pageTitle = ref('Students')
</script>
<template>
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                    ...
                    <button type="button" class="flex items-center justify-center text-white bg-primary hover:bg-primary focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-4 py-4">Matricule</th>
                    ...
                    <tbody >
                        <tr class="border-b dark:border-gray-700" v-for="student in data">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{student.matricule}}</th>
                            <td class="px-4 py-3">{{ student.nom }}</td>
                            <td class="px-4 py-3">{{ student.prenom }}</td>
                            <td class="px-4 py-3 max-w-[12rem]">{{ student.date_naissance }}</td>
                            <td class="px-4 py-3 max-w-[12rem] truncate">{{ student.adresse }}</td>
                            <td class="px-4 py-3">{{ student.phone }}</td>
                            <td class="px-4 py-3">{{ student.email }}</td>
                            <td class="px-4 py-3 flex items-center justify-end">

                              <router-link :to="{name:'StudentEdit', params:{id: student.id}}">
                                <button class="bg-primary text-white rounded-full p-1 text-xs">
                                e
                               </button>
                              </router-link>

                              <router-link :to="{name: 'StudentView', params:{id: student.id}}">
                               <button class="bg-warning text-white rounded-full p-1 text-xs m-2">
                                v
                               </button>
                            </router-link>

                            <router-link :to="{name:'StudentEdit', params:{id: student.id}}">
                               <button class="bg-danger text-white rounded-full p-1 text-xs">
                                d
                               </button>
                            </router-link>
